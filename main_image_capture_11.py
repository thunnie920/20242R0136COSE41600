...
for idx, pcd_file in enumerate(pcd_files):
    ...
    for cluster_idx, cluster in enumerate(clusters):
       ...
       # 클러스터 최소/최대값 계산 (한 번에 처리)
        ...
        # 조건을 만족하는 클러스터만 저장
        filtered_centroids.append(cluster_points.mean(axis=0))  # 중심점 저장
        filtered_bboxes.append(cluster.get_axis_aligned_bounding_box())  # 바운딩 박스 저장
    ...
    if idx < 50:
        ...
        # idx == 50일 때는 idx - 50과의 비교만 수행
        compare_idx = idx - 50
        if compare_idx in centroid_history:
            past_centroids = centroid_history[compare_idx]

            for curr_idx, curr_centroid in enumerate(filtered_centroids):
                distances_to_past = np.linalg.norm(past_centroids - curr_centroid, axis=1)
                closest_past_idx = np.argmin(distances_to_past)
                movement_50 = distances_to_past[closest_past_idx]

                if movement_50 > 1.0:  # 이동이 있는 경우 빨간색으로 표시
                    print(f"Movement detected for cluster {curr_idx} between frame {idx} and frame {compare_idx}: {movement_50}")
                    filtered_bboxes[curr_idx].color = (1, 0, 0)  # 이동이 있는 클러스터는 빨간색
                else:
                    filtered_bboxes[curr_idx].color = (0, 0, 1)  # 이동이 없는 경우 파란색 유지
        ...
        # 이전 프레임 데이터 가져오기
        if prev_idx in bbox_history:
            prev_bboxes = bbox_history[prev_idx]
            prev_centroids = [bbox.get_center() for bbox in prev_bboxes]  # 이전 bbox의 중심점
        ...
        for curr_idx, curr_centroid in enumerate(filtered_centroids):
            # 1. 직전 프레임 bbox의 중심점을 기준으로 탐지
            detected = False
            for prev_bbox in prev_bboxes:
                prev_center = prev_bbox.get_center()
                distance_to_prev_center = np.linalg.norm(curr_centroid - prev_center)

                if distance_to_prev_center < 1.0:  # 탐지 반경 내에 있으면 감지
                    detected = True
                    break

            if detected:
                # 2. 50 프레임 이전의 이동 여부 확인
                if compare_idx in centroid_history:
                    distances_to_past = np.linalg.norm(past_centroids - curr_centroid, axis=1)
                    closest_past_idx = np.argmin(distances_to_past)
                    movement_50 = distances_to_past[closest_past_idx]

                    if movement_50 > 1.0:  # 50 프레임 전과 비교하여 이동이 있는 경우
                        print(f"Movement detected for cluster {curr_idx} between frame {idx} and frame {compare_idx}: {movement_50}")
                        filtered_bboxes[curr_idx].color = (1, 0, 0)  # 이동이 있는 클러스터는 빨간색
                    else:
                        filtered_bboxes[curr_idx].color = (0, 0, 1)  # 이동이 없는 경우 파란색 유지
                else:
                    filtered_bboxes[curr_idx].color = (0, 0, 1)  # 비교 데이터가 없으면 기본값

            else:
                # 직전 프레임 bbox 주변에 탐지되지 않은 클러스터는 무시
                print(f"Cluster {curr_idx} is outside previous frame's bounding boxes.")
                continue

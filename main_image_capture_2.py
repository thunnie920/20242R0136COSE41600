...
output_folder = "E:\\Desktop\\selfdrivingCars\\COSE416_HW1\\visualized_pcd\\07_straight_walk_sample"  # 저장할 폴더
...
for idx, pcd_file in enumerate(pcd_files):
    ...
    if idx == 0:
        ...
        # 이전 프레임과 비교하여 움직임 감지
        if prev_centroids is not None and prev_centroids.size > 0:
            for curr_idx, curr_centroid in enumerate(current_centroids):
                ...
                # 움직임이 있으면 해당 바운딩 박스 업데이트
                if movement > movement_threshold:
                    print(f"Movement detected in cluster {curr_idx}: {movement}")
                    current_bboxes[curr_idx].color = (1, 0, 0)  # 움직임 있는 클러스터는 빨간색으로 표시

            ...
            if min_points_in_cluster <= len(cluster_indices) <= max_points_in_cluster:
                ...
                if min_height <= height_diff <= max_height and min_size <= xy_size <= max_size:
                    ...
                    if height_diff / xy_size >= aspect_ratio_threshold:
                        ...
                        bbox.color = (0, 0, 1)  # 임시 리스트 -> 파란색
                        temp_person_bboxes.append(bbox)
                        prev_centroids = current_centroids
                        prev_bboxes = current_bboxes
        visualize_with_bounding_boxes(final_point, temp_person_bboxes, output_path)

        # 현재 상태 저장
        prev_centroids = current_centroids
        prev_bboxes = temp_person_bboxes

    else:  # 이후 PCD 파일 처리
        # temp_person_bboxes에 있는 bbox 위주로 움직임 감지
        updated_bboxes = []
        updated_centroids = []
        print(f"{idx} 번째 frame입니다.")

        for temp_idx, temp_bbox in enumerate(prev_bboxes):
            # temp_person_bboxes의 중심점 계산
            temp_centroid = np.asarray(temp_bbox.get_center())

            # 현재 중심점들과의 거리 계산
            distances = np.linalg.norm(current_centroids - temp_centroid, axis=1)
            closest_curr_idx = np.argmin(distances)
            movement = distances[closest_curr_idx]
        
            if idx < 5:
                # 움직임이 movement_threshold 이상인 경우 해당 바운딩 박스를 업데이트 목록에 추가
                if movement > movement_threshold:
                    print(f"Movement detected in temp_person_bboxes {temp_idx}: {movement}")
                    temp_person_bboxes[temp_idx].color = (0, 0, 1)  # 파란색으로 표시
                    updated_bboxes.append(temp_person_bboxes[temp_idx])
                    updated_centroids.append(temp_centroid)
                else:
                    print(f"No significant movement in temp_person_bboxes {temp_idx}")
            
                # 업데이트된 바운딩 박스와 중심점 리스트로 갱신
                prev_centroids = np.array(updated_centroids)
                prev_bboxes = updated_bboxes
                # 시각화 및 결과 저장
                visualize_with_bounding_boxes(final_point, updated_bboxes, output_path)
        
            # 5번째 frame까지도 살아남은 temp_person을 진짜 사람으로 인지하기
            else:
                # 움직임이 movement_threshold 이상인 경우 해당 바운딩 박스를 업데이트 목록에 추가
                if movement > movement_threshold:
                    print(f"Movement detected in temp_person_bboxes {temp_idx}: {movement}")
                    temp_person_bboxes[temp_idx].color = (1, 0, 0)  # 빨간색으로 표시
                    updated_bboxes.append(temp_person_bboxes[temp_idx])
                    updated_centroids.append(temp_centroid)
                else:
                    print(f"No significant movement in temp_person_bboxes {temp_idx}")
